...
interface FlightFilterContextType {
  ...
  // Whether any filter controls are actually meaningful/available
...
export function FlightFilterProvider({
  children,
  flightData,
}: FlightFilterProviderProps) {
  // Calculate metadata from flight data
  const priceStats = useMemo(
    () => getFlightPriceStats(flightData),
    [flightData],
  );
  const maxAvailableStops = useMemo(
    () => getMaxStopsFromFlights(flightData),
    [flightData],
  );
  const availableAirlines = useMemo(
    () => getAirlinesFromFlights(flightData),
    [flightData],
  );
  ...
  const activeFiltersCount = useMemo(() => {
    ...
    maxStops,
    maxAvailableStops,
    selectedDepartureTime.length,
    priceRange,
    priceStats,
  ]);

  // Determine if any filter UI should be shown at all
  const hasAvailableFilters = useMemo(() => {
    const hasPriceRange = priceStats.max > priceStats.min;
    const hasAirlines = availableAirlines.length > 1;
    const hasStopsChoice = maxAvailableStops > 0;
    // Departure time checkboxes exist regardless of data spread; rely on core three
    return hasPriceRange || hasAirlines || hasStopsChoice;
  }, [
    priceStats.max,
    priceStats.min,
...
export function useFlightFilter() {
  ...
  if (context === undefined) {
    throw new Error(
      "useFlightFilter must be used within a FlightFilterProvider",
    );
  }
  return context;
}
